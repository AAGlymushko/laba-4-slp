...
export class Enemy extends GameObject
{
    ...
    paint()
    {
        ...
        const img = ["Ниндя 2.png", "Ниндя 1.png",  "Ниндя 3.png", "Ниндя 1.png"];
        
        const frameIndex = Math.floor(this.#cadr) % img.length;
        sprite.src = img[frameIndex];

        if (this.#is_right) ctx.drawImage(sprite, x, y, w, h);
        else 
        {
            ctx.save();
            ctx.translate(x + w, y);
    ...
    update()
    {
        ...
        if (Math.round(Math.random() * this.#rate_correct) === this.#rate_correct) this.#is_right = !this.#is_right;
        
        this.#cadr += this.#animationSpeed;
        if (this.#cadr > 4) this.#cadr = 0;
